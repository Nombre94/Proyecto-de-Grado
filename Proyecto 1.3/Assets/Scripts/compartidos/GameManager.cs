
// este script es el encargado de establecer las variables que se compartiran entre scripts
public class GameManager
{
    public static float volmusic;
    public static int acion;
    public static int nivel;
    public static float velocidad;
    public static float caidabase;
    public static float velcaida;
    public static float delaybase;
    public static float delay;
    public static float delayobs=3;
    public static int borrado;
    public static int tiempo;
    public static int enter;
    public static int fin;
    public static int codigo;
    public static int limite=0;
    public static int acierto;
    public static int limitepalabras=0;
    public static int final;
    public static int muerte;
    public static int contaciertos;
    public static int conterrores;
    public static int contperdidas;
    public static int random;
    
    
}


